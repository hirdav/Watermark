...
import { deleteFileFromStorage, readFileFromStorage } from "@/lib/storage";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ imageId: string }> }) {
  ...
  await deleteFileFromStorage(image.originalPath);
  await deleteFileFromStorage(image.watermarkedPath);
